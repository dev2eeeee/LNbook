...
앨리스는 자신의 비트코인 소유권을 제3자에게 위임하고 싶지 않았어요. 그녀는 어떻게 지갑을 사용하는지 알고 있을 정도로 암호화폐에 대해 많이 공부해왔죠. 또, 그녀는 이동 중에도 소액 결제가 가능한 모바일 지갑을 원했습니다. 그래서 비수탁형(개인키를 직접 보관하여 단독 제어하는) 모바일 라이트닝 지갑인 _Eclair_ 지갑을 선택했습니다. 이제 어떻게, 그리고 왜 그녀가 이 선택을 했는지 알아봅시다.

=== 라이트닝 노드들

((("Lightning node operation")))라이트닝 네트워크는 LN 프로토콜이라고 부르는 소프트웨어 애플리케이션을 통해 접근할 수 있습니다. _라이트닝 네트워크 노드_ (LN 노드나 간단한 노드)는 세 가지 중요한 특성을 가진 소프트웨어 애플리케이션입니다. 첫번째로, 라이트닝 노드들은 지갑이기 때문에 라이트닝이나 비트코인 네트워크에서 지불하거나, 지불 받을 수 있습니다. 두번째로, 노드는 P2P를 기반으로 다른 라이트닝 노드들과 네트워크를 만들면서 소통해야만 합니다. 마지막으로, 라이트닝 노드들은 지불할 때 사용한 자금을 확보하기 위해 비트코인 블록체인(혹은 다른 암호화폐 블록체인)에 접근이 필요합니다. 

사용자들은 자신만의 비트코인과 라이트닝 노드 모두를 운영하여 최고 수준의 제어를 할 수도 있습니다. 혹은, ((("simplified payment verification (SPV)")))((("SPV (simplified payment verification)")))라이트닝 노드들은 비트코인 블록체인과 상호작용하기 위해 SPV(Simplified Payment Verification)라고 불리는, 가벼운 비트코인 클라이언트를 사용할 수 있습니다.

[[ln_explorer]]
=== 라이트닝 익스플로러

((("Lightning explorers")))LN 익스플로러는 노드, 채널들, 네트워크 용량의 통계를 보여주는 유용한 툴입니다.

다음은 포괄적인 목록입니다.

* https://1ml.com[1ML Lightning explorer] 
* https://explorer.acinq.co[ACINQ's Lightning explorer], 디자인이 팬시함.
* https://amboss.space[Amboss Space Lightning explorer], 커뮤니티와 직관적인 pass:[<span class="keep-together">디자인</span>]
* https://ln.bigsun.xyz[Fiatjaf's Lightning explorer], 다이어그램이 많음.
*  https://hashxp.org/lightning/node[hashXP Lightning explorer]

[TIP]
====
다른 블록체인처럼 라이트닝 익스플로러를 사용하실 때, 프라이버시가 중요하다는 점을 아셔야 합니다. 사용자가 부주의할 경우, 웹사이트는 여러분의 IP를 추적하여, 여러분의 동작을 기록할 수 있습니다(예를 들어, 사용자가 관심을 갖는 노드들).

또, 현존하는 라이트닝 그래프나 채널 정책의 상태는 아직 세계적으로 합의된 바가 없습니다. 그래서 사용자들은 정보를 얻으려고 라이트닝 익스플로러에 절대 많이 의존하면 안됩니다. 
게다가, 사용자들이 채널을 열고, 닫고, 업데이트 할 때마다, 그래프가 바뀌거나 혹은 각 라이트닝 익스플로러는 최신 업데이트가 안될 수도 있습니다. 네트워크 정보를 시각화할 때만 라이트닝 익스플로러를 사용하세요. 라이트닝 네트워크에서 무슨 일이 일어났는지를 권위 있는 출처로 쓰지 마십시오. 라이트닝 네트워크를 제대로 보려면, 여러분이 직접 채널 그래프와 다양한 통계를 모아 웹 기반 인터페이스로 볼 수 있는 라이트닝 노드를 돌리십시오.
====
